...
import dev.ssjvirtually.util.XmlConverterUtil;
...
public class XmlJsonConverterView extends VBox {
    ...
    public XmlJsonConverterView() {
        ...
        inputArea = createEditor("Enter XML or JSON...");
        outputArea = createEditor("Result appears here...");
        outputArea.setEditable(false);

        HBox config = new HBox(10);
        Button xmlToJsonBtn = new Button("XML to JSON");
        xmlToJsonBtn.setOnAction(e -> convertXmlToJson());
        Button jsonToXmlBtn = new Button("JSON to XML");
        jsonToXmlBtn.setOnAction(e -> convertJsonToXml());
        config.getChildren().addAll(xmlToJsonBtn, jsonToXmlBtn);
    ...
    private void convertXmlToJson() {
        try {
            String result = XmlConverterUtil.xmlToJson(inputArea.getText());
            outputArea.replaceText(result);
            clearError();
        } catch (Exception ex) {
            showError("XML to JSON error: " + ex.getMessage());
        }
    }

    private void convertJsonToXml() {
        try {
            String result = XmlConverterUtil.jsonToXml(inputArea.getText());
            outputArea.replaceText(result);
            clearError();
        } catch (Exception ex) {
            showError("JSON to XML error: " + ex.getMessage());
        }
    }

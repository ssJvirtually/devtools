...
import dev.ssjvirtually.util.SqlUtil;
...
public class SqlFormatterView extends VBox {
    ...
    public SqlFormatterView() {
        ...
        Label title = new Label("SQL Formatter");
        ...
        inputArea = createEditor("Enter SQL...");
        outputArea = createEditor("Formatted SQL appears here...");
        outputArea.setEditable(false);

        HBox config = new HBox(10);
        Button formatBtn = new Button("Format SQL");
        formatBtn.setOnAction(e -> format());
        config.getChildren().addAll(formatBtn);
    ...
    private void format() {
        try {
            String result = SqlUtil.format(inputArea.getText());
            outputArea.replaceText(result);
            clearError();
        } catch (Exception ex) {
            showError("SQL Format error: " + ex.getMessage());
        }
    }
